...
export default function Pagination({ currentPage, totalPages, onPageChange, darkMode = false }) {
    ...
    return (
      <nav className={`isolate inline-flex -space-x-px rounded-md shadow-sm ${darkMode ? "border-neutral-800" :"border-neutral-300"}`} aria-label="Pagination">
        ...
        <p className={`px-4 py-2 text-sm font-semibold border ${darkMode ? "border-neutral-800" :"border-neutral-300"}`}>Page {currentPage + 1} de {totalPages}</p>
